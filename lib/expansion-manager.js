...
const { DefaultTransformResolver } = require("./transform-resolver"); // TODO: Move this to same area as snippet management
...
class ExpansionManager {
  ...
  expandSnippetWithPrefix(insertionRange, prefix, parser) {
    const scope = this.editor.scopeDescriptorForBufferPosition(insertionRange.end);
    ...
    const selectionRange = this.editor.getLastCursor().selection.getBufferRange();
    ...
    const expansion = new Expansion(snippet.getTree(parser), context, insertionRange, this.markerLayer);
  ...
  expandSnippetsUnderCursors(parser) {
    ...
    const prefixCandidate = this.editor.getTextInBufferRange([cursorPosition.translate(new Point(0, -50)), cursorPosition]);
  ...
  registerExpansion(expansion) {
    if (!expansion.finished) {
      ...
      this.snippets.notifyChangeExpansionState(this.editor, this.hasActiveExpansions());
  ...
  getContext(cursor, prefix) {
    ...
    return {
      ...
      transformResolver: new DefaultTransformResolver(),
