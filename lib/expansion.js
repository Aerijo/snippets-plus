...
class Expansion {
  ...
  gotoIndex(index) {
    if (this.activeStopIndex >= 0) {
      const activeInstances = this.tabStopsByGroup[this.activeStopIndex].instances;
  ...
  function resolveVariable(variableItem) {
    ...
    if (typeof resolved === "string") {
      if (variableItem.transformation) {
        const transformed = resolveVariableTransformation(variableItem.transformation, resolved);
