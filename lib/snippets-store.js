...
class SnippetsStore {
  ...
  async add(packageSnippets) {
    console.log(`Adding snippets for ${packageSnippets.getName()}`);

    const priority = packageSnippets.getPriority();
    const packageName = packageSnippets.getName();
    const snippetsByFile = await packageSnippets.getSnippets();
    ...
    for (const snippetsBySelector of snippetsByFile.values()) {
      for (const [selector, snippets] of snippetsBySelector) {
        ...
        this.addForSelector(selector, packageName, priority, snippets);
      }
    }
  ...
  remove(packageSnippets) {
    console.log(`Removing snippets for ${packageSnippets.getName()}`);
    const packageName = packageSnippets.getName();

    for (const [selector, snippetPools] of this.snippetsBySelector) {
      let i = 0;
      for (; i < snippetPools.length; i++) {
        if (snippetPools[i].name === packageName) {
          break;
        }
      }
  ...
  addForSelector(selector, name, priority, snippets) {
...
function getOrCompute(container, key, defaultValue) {
