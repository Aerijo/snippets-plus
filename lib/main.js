...
const { Emitter, File } = require("atom");
...
class SnippetsPlus {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.commands.add("atom-text-editor", {
      "snippets:expand": function(event) {
        ...
        if (!snippets.expandSnippetsUnderCursors(editor, snippets.getSnippetParser())) {
      ...
      "snippets:cancel": function(event) {
        const editor = this.getModel();
        return snippets.cancelExpansions(editor);
      }
    }));
  }

  expandSnippetsUnderCursors(editor, parser) {
    const expManager = getOrCompute(this.expansionsByEditor, editor, () => new ExpansionManager(this, editor));
  ...
  async observeAll() {
    ...
    this.observeUserSnippets(path.join(atom.getConfigDirPath(), "snippets.cson")).then(() => {
      console.log("Completed observing user snippets");
    });
  }
  ...
  observeUserSnippets(userSnippetsPath) {
    ...
    this.subscriptions.add(userSnippets.onDidChange(() => {
...
class PackageSnippets extends SnippetsGroup {
  constructor(pack, enabled) {
    super({
      ...
      enabled,
      active: false,
      priority: atom.packages.isBundledPackage(pack.name) ? Priority.Bundled : Priority.Community,
    });
  }
...
class UserSnippets extends PackageSnippets {
  ...
  async generateSnippets() {
    return new Map([
      [this.snippetsPath, buildSnippets(await readAndGetObjectData(this.snippetsPath))],
    ]);
  }
  ...
  observeChanges() {
    ...
    try {
      this.subscriptions.add(file.onDidChange(() => this.emitter.emit("change", this)));
      this.subscriptions.add(file.onDidDelete(() => this.emitter.emit("change", this)));
      this.subscriptions.add(file.onDidRename(() => this.emitter.emit("change", this)));
    } catch (e) {
      console.error(e);
    }
  }
...
class TestSnippets extends SnippetsGroup {
  ...
  async generateSnippets() {
    return new Map([
      ["testSnippets", this.testSnippets],
    ]);
  }
}
