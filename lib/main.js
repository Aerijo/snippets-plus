...
class SnippetsPlusPackage {
  ...
  consumeSnippetsResolver({ transformResolver, variableResolver }) {
    if (transformResolver) {
      this.consumeSnippetsTransformResolver(transformResolver);
    }

    if (variableResolver) {
      this.consumeSnippetsVariableResolver(variableResolver);
    }
  }

  consumeSnippetsVariableResolver({ resolver, priority }) {
    this.snippets.variableResolver.addResolver(resolver, priority);
  }

  consumeSnippetsTransformResolver({ resolver, priority }) {
    this.snippets.transformResolver.addResolver(resolver, priority);
  }
