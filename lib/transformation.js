      ...
      function processReplacements(replacements) {
        for (const segment of replacements) {
          ...
          if (segment.backreference !== undefined) {
            ...
            const capture = typeof group === "number"
              ? match[group]
              : match[match.length - 1][group];
            ...
            if (capture === undefined) {
              ...
              const text = segment.modifier !== undefined
                ? context.transformResolver.resolve(segment.modifier, text, context)
                : capture;
